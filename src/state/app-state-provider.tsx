...
type QuestionGenerationMode = 'create' | 'addBelow' | 'transform';
...
const EditorStateContext = createContext<LumiEditorState | null>(null);
...
function createTextContent(text = ''): TextContent {
  ...
  const actions = useMemo<AppActions>(
    () => ({
      chatMessageAdded: (message) =>
        setChatState((prev) => ({ ...prev, messages: [...prev.messages, message] })),
      ...
      chatPreviewUpdated: (markdown, doc) =>
        setChatState((prev) => ({ ...prev, preview: markdown, previewDoc: doc })),
      chatReadAloudToggled: () =>
        setChatState((prev) => ({ ...prev, readAloudEnabled: !prev.readAloudEnabled })),
      ...
      apiEndpointChanged: (apiEndpoint) =>
        setEditorState((prev) => ({ ...prev, apiConfig: { ...prev.apiConfig, apiEndpoint } })),
      apiTokenChanged: (apiToken) =>
        setEditorState((prev) => ({ ...prev, apiConfig: { ...prev.apiConfig, apiToken } })),
      apiModelChanged: (apiModel) =>
        setEditorState((prev) => ({ ...prev, apiConfig: { ...prev.apiConfig, apiModel } })),
      ...
      worksheetReset: () =>
        setEditorState((prev) => ({ ...getInitialEditorState(), apiConfig: prev.apiConfig })),
      ...
      worksheetContentAdded: ({ content, index }) =>
        setEditorState((prev) => {
          ...
          return { ...prev, content: { ...prev.content, [content.id]: content }, structure };
        }),
      worksheetContentUpdated: ({ id, updates }) =>
        setEditorState((prev) => {
          ...
          return {
            ...prev,
            content: { ...prev.content, [id]: { ...current, ...updates } as Content },
          };
        }),
      ...
      worksheetContentDuplicated: (id) =>
        setEditorState((prev) => {
          ...
          return { ...prev, content: { ...prev.content, [newId]: duplicate }, structure };
        }),
      ...
      contentLoadingSet: ({ contentId, loading }) =>
        setEditorState((prev) => ({
          ...prev,
          ui: { ...prev.ui, loading: { ...prev.ui.loading, [contentId]: loading } },
        })),
      worksheetSavingSet: (saving) =>
        setEditorState((prev) => ({ ...prev, ui: { ...prev.ui, saving } })),
      tokenLimitErrorSet: (message) =>
        setEditorState((prev) => ({ ...prev, ui: { ...prev.ui, tokenLimitError: message } })),
      tokenLimitErrorCleared: () =>
        setEditorState((prev) => ({ ...prev, ui: { ...prev.ui, tokenLimitError: null } })),
      sendMessage,
      generateQuestion,
      generateText,
      sendChatMessage,
