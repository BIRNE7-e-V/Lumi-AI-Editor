...
function ContentSection({ item, isLast }: { item: Content; isLast: boolean }) {
  ...
  if (item.type === 'text') {
    rendered = <TextItem text={item.text} />;
  } else if (item.type === 'multiple-choice') {
    rendered = <MCQItem answers={item.answers} question={item.question} />;
  } else if (item.type === 'fill-in-the-blanks') {
    rendered = <FillInTheBlanksItem text={item.text} />;
  } else {
    rendered = <FreetextItem task={item.task} />;
  }
