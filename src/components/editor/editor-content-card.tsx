...
import { memo, useCallback, useEffect, useState, type DragEvent, type ReactNode } from 'react';
...
function ContentTypeMenu({
  ...
  return (
    <details className="dropdown dropdown-end">
      ...
      <ul className="menu dropdown-content z-20 mt-2 w-56 rounded-box border border-base-300 bg-base-100 p-2 shadow-lg">
  ...
  return (
    <div className="relative" onDragOver={onDragOver} onDrop={onDrop}>
      {isDropTarget && dropPosition === 'before' ? (
        <div className="absolute inset-x-0 -top-1 z-10 h-1 rounded-full bg-primary" />
      ) : null}

      <article
        className={twMerge(
          'rounded-box border border-base-300 bg-base-100 p-4 shadow-sm transition-colors',
      ...
      >
        <div className="mb-4 flex items-start justify-between gap-3">
          <div className="flex items-center gap-2">
            <div
              className={twMerge(
                'badge badge-outline text-xs badge-xl',
      ...
      {isDropTarget && dropPosition === 'after' ? (
        <div className="absolute inset-x-0 -bottom-1 z-10 h-1 rounded-full bg-primary" />
      ) : null}
    </div>
  );
});
