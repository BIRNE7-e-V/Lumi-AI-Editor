  ...
  return (
    <label className="form-control gap-2 w-full">
      <div className="space-y-1">
        ...
        {hint && <p className="text-xs leading-5 text-base-content/60">{hint}</p>}
      </div>
      {children}
    </label>
  );
...
export const SettingsModal = memo(function SettingsModal({
  ...
  }, [chat.customSystemPrompt, open]);

  return (
    <Modal open={open} title="KI-Einstellungen" onClose={onClose}>
      <div className="space-y-6">
        <div className="flex gap-4">
          <div className="rounded-box border border-base-300 bg-base-200/70 p-4 w-full">
        ...
        <div className="rounded-box border border-base-300 bg-base-200/70 p-4">
          ...
          {Object.keys(usageByModel).length === 0 ? (
            <p className="text-xs text-base-content/40">Noch keine Anfragen gesendet.</p>
          ) : (
            <>
              {Object.entries(usageByModel).map(([modelId, m]) => {
                ...
                return (
                  <div key={modelId} className="mb-3 last:mb-0">
                    <p className="text-xs font-medium text-base-content/70 mb-1">{label}</p>
              ...
              {Object.keys(usageByModel).length > 1 &&
                (() => {
                  ...
                  return (
                    <div className="border-t border-base-300 pt-2 mt-2 flex items-baseline gap-2">
                      <span className="text-xs text-base-content/60">Gesamt:</span>
