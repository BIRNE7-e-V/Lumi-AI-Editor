...
const EmptyPreview = memo(function EmptyPreview() {
  return (
    <div className="flex h-full items-center justify-center rounded-box bg-base-200/80 p-6 text-center text-sm leading-6 text-base-content/70">
...
const PreviewItem = memo(function PreviewItem({ item }: { item: Content }) {
  ...
  return (
    <section className="space-y-4 rounded-box border border-base-300 bg-base-100 p-4">
      ...
      <p className="leading-7 text-base-content/80">{item.task || '(Freitextaufgabe folgt)'}</p>
      <div className="space-y-2 pt-2">
        {Array.from({ length: 4 }).map((_, index) => (
          <div key={`${item.id}-line-${index}`} className="h-4 rounded-full bg-base-300/70" />
        ))}
      </div>
    </section>
  );
...
export const PreviewSidebar = memo(function PreviewSidebar() {
  ...
  return (
    <aside className="flex h-full min-h-0 flex-col overflow-hidden bg-base-100 shadow-sm">
      <header className="border-b border-base-300 px-5 py-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-base-content/60">Schnelle Lesefassung und PDF-Export.</p>
          </div>
        </div>
      </header>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5">
        {!hasContent ? (
          ...
          <div className="space-y-4 rounded-box bg-base-200/80 p-4">
            {deferredTitle ? (
              <div className="rounded-box border border-base-300 bg-base-100 px-5 py-4">
                <h1 className="font-[Barlow] text-2xl font-bold text-base-content">
                  {deferredTitle}
                </h1>
              </div>
            ) : null}
