...
export function Modal({ open, title, description, size = 'lg', onClose, children }: ModalProps) {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center bg-base-content/25 p-4 backdrop-blur-sm"
      role="presentation"
      onMouseDown={(event) => {
    ...
    >
      <div
        className={`w-full rounded-box border border-base-300 bg-base-100 shadow-2xl ${size === 'md' ? 'max-w-2xl' : 'max-w-3xl'}`}
      >
