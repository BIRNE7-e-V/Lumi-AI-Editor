...
let skipNativeStt = typeof localStorage !== 'undefined' && localStorage.getItem(NATIVE_STT_SKIP_KEY) === '1';
...
export function useSpeech({
  ...
  const ensureStream = useCallback(async () => {
    ...
    const promise = (async () => {
      try {
        ...
        console.debug(`[speech] ${t()} ensureStream: getUserMedia resolved in ${(performance.now() - t0).toFixed(0)} ms`);
        ...
        console.error(`[speech] ${t()} ensureStream: getUserMedia failed after ${(performance.now() - t0).toFixed(0)} ms`, error);
  ...
  const startMediaRecorderListening = useCallback(async () => {
    console.debug(`[speech] ${t()} startMediaRecorderListening called`, { mediaRecorderSttAvailable, hasRecorder: !!recorderRef.current });
  ...
  const startNativeListening = useCallback(() => {
    console.debug(`[speech] ${t()} startNativeListening called`, { NativeSpeechRecognition: !!NativeSpeechRecognition, hasRecognition: !!recognitionRef.current });
    ...
    recognition.onerror = (event: SpeechRecognitionErrorEvent) => {
      ...
      if ((event.error === 'service-not-allowed' || event.error === 'network') && mediaRecorderSttAvailable) {
        ...
        console.debug(`[speech] ${t()} native STT unavailable, falling back to MediaRecorder + Whisper`, event.error);
        skipNativeStt = true;
        try { localStorage.setItem(NATIVE_STT_SKIP_KEY, '1'); } catch { /* ignore */ }
    ...
    recognition.onend = () => {
      console.debug(`[speech] ${t()} native recognition ended`, { fallback: nativeSttFallbackRef.current });
  ...
  const startListening = useCallback(async () => {
    ...
    if (skipNativeStt) {
      console.debug(`[speech] ${t()} startListening: native STT skipped (previously failed), using MediaRecorder`);
    }
